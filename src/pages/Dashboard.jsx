...
const iconBgMap = {
  ...
  emerald: "bg-[#27AE60] text-white",
  amber: "bg-[#F5A623] text-white",
  rose: "bg-[#E74C3C] text-white",
...
const StatCard = ({ title, value, icon: Icon, trend, color, delay, onClick }) => <motion.div
  ...
  onClick={onClick}
  className="bg-white p-4 sm:p-5 rounded-xl border border-gray-200 relative overflow-hidden group cursor-pointer shadow-nexi5"
>
  <div className="flex justify-between items-start relative z-10">
    <div className="min-w-0 flex-1">
      <p className="text-[11px] sm:text-xs font-semibold text-textSecondary uppercase tracking-wider truncate">{title}</p>
    ...
    <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-full shrink-0 ${iconBgMap[color] || iconBgMap.primary} flex items-center justify-center transition-transform duration-300 group-hover:scale-110 ml-3 shadow-sm`}>
      <Icon size={22} />
    </div>
  </div>
</motion.div>;
...
function Dashboard() {
  ...
  const QuickActions = ({ actions }) => (
    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-nexi5">
        ...
        {actions.map((action, i) => (
          <button
            ...
            className={`flex flex-col items-center gap-2 p-4 rounded-xl transition-all ${
              action.primary
                ? 'bg-gradient-to-br from-[#0f4184] to-[#0b3166] text-white hover:opacity-90 shadow-md col-span-2'
                : 'border border-gray-200 text-slate-600 hover:bg-slate-50 hover:border-primary/30 hover:text-primary'
            }`}
          >
            {action.primary ? (
              <>
              ...
              <>
                ...
                <span className="text-xs font-semibold uppercase tracking-tight">{action.label}</span>
              </>
            )}
          </button>
        ))}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-textPrimary tracking-tight">
            <span className="text-primary">{userRole || "Admin"}</span> Dashboard
          </h1>
          <p className="text-sm text-textSecondary mt-0.5">Comprehensive overview for {userRole || "Admin"}.</p>
        </div>
        <span className="hidden sm:inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-primary/10 text-primary border border-primary/20">
          {userRole || "Admin"} Perspective
        </span>
      </div>
